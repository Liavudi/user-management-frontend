...
export class RegistrationComponent implements OnInit {
  ...
  onSubmit(userId: any, userAge: string, name: string, password: string, email: string, userName: string) {
    ...
    this.usersService.createUser(userDetails).subscribe(res => { if (res.status === 201) { alert(res.body?.data) } else { alert(`Error creating user. Reason: ${res.body?.error.message}`) } });
